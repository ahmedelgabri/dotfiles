...
-- SchemaStore: just needs to be installed (used by jsonls/yamlls in after/lsp/)
...
vim.api.nvim_create_autocmd('LspAttach', {
	desc = 'Load nvim-navic on first LSP attach',
	callback = function(ev)
		...
		-- Manually attach for this event (auto_attach handles future ones)
...
-- none-ls: lazy on LspAttach
local nonels_loaded = false
vim.api.nvim_create_autocmd('LspAttach', {
	desc = 'Load none-ls on first LSP attach',
	once = true,
	callback = function()
		if nonels_loaded then
			return
		end
		nonels_loaded = true
		vim.pack.add {
...
-- nvim-lspconfig: load after startup (was event = 'VeryLazy')
