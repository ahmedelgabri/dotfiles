...
async function showTotals(
	...
	if (!ctx.hasUI) {
		...
		for (const b of totals.byMonth) {
			plain.push(
				`${b.month}  ${fmtMoney(b.cost).padStart(10)}  ${fmtInt(b.messages).padStart(8)} msgs  ${fmtInt(b.sessions.size).padStart(4)} sessions`,
			)
		}
		plain.push('-------------------')
		plain.push(
			`Total     ${fmtMoney(totals.cost).padStart(10)}  ${fmtInt(totals.messages).padStart(8)} msgs  ${fmtInt(totals.sessions).padStart(4)} sessions`,
		)
